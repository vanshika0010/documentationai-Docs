...
<Callout kind="info" collapsed="false">
  Raise disputes within **3 days from the delivery date**. If the seller contacts support after 72 hours, assign the case to the **LOPS Dispute team as an exception** and guide the seller to raise future disputes within the timeline.
</Callout>
...
| Scenario                                                        | Action                                                                                                                                                                                                                                                                                                                                            |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Case 1: Seller reaches out same day                             | Ask seller to wait until end of day. POD will be updated on the panel by EOD.                                                                                                                                                                                                                                                                     |
| Case 2: Seller reaches out next day or later, POD available     | Check whether the seller verified the POD. If checked and shipment not received, create a new ticket on the dispute group and share a 5–7 working day investigation timeline. If POD is available but the seller has not checked it, ask the seller to verify POD during the call, take confirmation, and create FCR or NFCR based on seller VOC. |
| Case 3: Seller reaches out next day or later, POD not available | Assign to the LOPS Dispute team. Ask the seller to wait 5–7 working days for investigation.                                                                                                                                                                                                                                                       |

### If Status is OFD

| Scenario              | Action                                              |
| --------------------- | --------------------------------------------------- |
| OFD on current day    | Ask seller to wait till EOD for final status update |
| OFD from previous day | Follow the standard DNU process above               |
...
Use this SOP for product receipt disputes where the shipment was delivered but item condition, quantity, or contents do not match expectations.
...
### Buyer Confirmation Checklist (BPR)
...
<Callout kind="alert" collapsed="false">
  If the outer packaging is intact but the product is damaged from inside, deny the claim. Packaging responsibility rests with the seller.
</Callout>

| Scenario                                       | Action                      |
| ---------------------------------------------- | --------------------------- |
| Outer packaging intact, product damaged inside | Deny the claim              |
...
Applies to Forward, RTO, and Reverse shipments where the shipment is marked delivered but the recipient confirms non-receipt.
...
| OFD from previous day | Follow the DNR process based on order value   |
